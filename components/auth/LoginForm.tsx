    ...
    <div className="font-display bg-background-light dark:bg-background-dark min-h-screen flex items-center justify-center p-4 antialiased text-nordic dark:text-gray-100 relative overflow-hidden">
      ...
      <main className="w-full max-w-md z-10">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-xl mb-6 overflow-hidden">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/logo.svg" alt="Luxe Estate Logo" className="w-full h-full object-contain" />
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-nordic dark:text-white mb-2">{dict?.login?.title || 'Welcome to Luxe Estate'}</h1>
        ...
        <div className="bg-surface dark:bg-[#152e2a] rounded-2xl shadow-soft p-8 sm:p-10 border border-white/50 dark:border-primary/20 backdrop-blur-sm">
          <div className="space-y-4">
            ...
            >
              ...
              <span className="relative z-10">{dict?.login?.continue_with_google || 'Continue with Google'}</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="relative z-10">{dict?.login?.continue_with_github || 'Continue with GitHub'}</span>
            </button>
          </div>
          <p className="mt-8 text-center text-sm text-nordic/70 dark:text-gray-400">
            {dict?.login?.no_account || "Don't have an account?"} 
            <Link className="font-semibold text-primary hover:text-primary-dark transition-colors ml-1" href={`/${lang}/login`}>{dict?.login?.sign_up || 'Sign up'}</Link>
          </p>
        </div>
        <div className="mt-8 text-center">
          <nav className="flex justify-center gap-6 text-xs text-nordic/50 dark:text-gray-500">
            <Link className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{dict?.login?.privacy_policy || 'Privacy Policy'}</Link>
            <Link className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{dict?.login?.terms_of_service || 'Terms of Service'}</Link>
            <Link className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{dict?.login?.help_center || 'Help Center'}</Link>
          </nav>
        </div>
      </main>
    </div>
